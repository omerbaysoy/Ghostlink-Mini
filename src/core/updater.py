...
def update_ghostlink():
    ...
    out, code = _run(_sudo_args(["./setup.sh", "--update"]))
    if code != 0:
        return {
            "status": "error",
            "message": "Update setup failed. Run `sudo ./setup.sh --update` from /opt/ghostlink-mini for an interactive retry.",
            "log": out,
        }
